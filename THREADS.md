...
- 답글 대상 작성자/본문 미리보기(차단·삭제 시 숨김), 알림으로 연 답글이 40번째 이후여도 바로 표시
- 안 읽은 알림 필터, 화면에 불러온 알림만 ID별로 묶어 읽음 처리
- 편집 중 다른 창에서 수정된 글을 덮어쓰지 않도록 충돌 안내, 작성 글자 수 표시

브라우저 저장소가 차단되거나 용량이 찬 경우에도 현재 창 안에서는 ID별 초안을 메모리에 보존합니다. 이 경우 새로고침/창 닫기 이후의 보존은 보장되지 않습니다. 일괄 읽음은 표시된 알림만 대상으로 하며 아직 불러오지 않은 알림은 읽음 처리하지 않습니다.
...
| POST read | id(알림 ID) |
| POST read_many | ids(같은 수신 ID의 알림 1~40개, 전체 소유권 검증 후 원자적 처리) |

`GET notifications`는 `unread=1`로 안 읽은 알림만 반환합니다. `GET detail`은 요청한 답글이 첫 페이지 밖에 있을 때 `focused_reply`로 해당 답글을 추가 반환합니다. `POST edit`의 `originalBody`가 현재 본문과 다르면 409를 반환합니다. 새 UI는 이 값을 항상 전달합니다. 이 보완판은 새로운 SQL 마이그레이션 없이 기존 `threads-schema.sql`을 그대로 사용합니다.
