...
import { ATTACHMENT_UPLOAD_BYTES_MAX, IMAGE_DIMENSION_MAX, IMAGE_EDGE_PIXELS_MAX, IMAGE_FALLBACK_TOTAL_PIXELS_MAX, IMAGE_OPTIMIZE_BYTES_MIN, IMAGE_REQUIRED_SAVINGS_RATIO, IMAGE_SOURCE_BYTES_MAX, IMAGE_TOTAL_PIXELS_MAX, IMAGE_WEBP_QUALITY, api, chatAttachmentButton, chatAttachmentGuide, chatAttachmentGuideItems, chatAttachmentInput, chatAttachmentName, chatAttachmentPreview, chatAttachmentTray, registerCoreHooks, requestAction, roomSettingsSheet, setAppStatus, state } from "./core.js";
import { decodedImageSize } from "./profile.js";
import { cancelVoiceRecording, chatVoicePreview } from "./voice.js";
...
const ATTACHMENT_TYPES_BY_EXTENSION = Object.freeze({
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
  png: "image/png",
  gif: "image/gif",
...
const GENERAL_FILE_EXTENSIONS = ["txt", "csv", "md", "rtf", "zip", "doc", "xls", "ppt", "docx", "xlsx", "pptx"];
const GENERAL_FILE_ACCEPT = GENERAL_FILE_EXTENSIONS.map((extension) => `.${extension}`).join(",");
const ALL_ATTACHMENT_ACCEPT = `image/*,application/pdf,${GENERAL_FILE_ACCEPT}`;
...
function openAttachmentPicker(kind = "all") {
  if (kind !== "all" && kind !== "photo" && kind !== "pdf" && kind !== "file") {
    setAppStatus("준비 중인 기능이에요.");
    return;
  }
  chatAttachmentInput.accept = kind === "pdf"
    ? "application/pdf"
    : (kind === "photo" ? "image/*" : (kind === "file" ? GENERAL_FILE_ACCEPT : ALL_ATTACHMENT_ACCEPT));
...
function renderChatAttachmentPreview() {
  ...
  const attachmentLabel = state.chatAttachmentType === "application/pdf"
    ? "PDF"
    : (state.chatAttachmentType.startsWith("image/") ? "사진" : "파일");
  chatAttachmentPreview.classList.toggle("hidden", !file);
  chatAttachmentName.textContent = file
    ? (isVoice ? `음성 메시지 · ${formatVoiceDuration(state.chatAttachmentDurationMs)}` : `${attachmentLabel}: ${file.name}`)
    : "";
...
function attachmentContentType(file) {
  ...
  return ATTACHMENT_TYPES_BY_EXTENSION[extension] || "";
}
...
async function selectChatAttachment(file) {
  ...
  if (!contentType) {
    clearChatAttachment();
    setAppStatus("사진, PDF 또는 지원되는 일반 파일만 보낼 수 있어요.", "error");
    return false;
  }
...
registerCoreHooks({ clearChatAttachment });
...
export {
  ...
  showAttachmentGuide,
  startChatAttachmentUpload,
  updateAttachmentSwipe,
