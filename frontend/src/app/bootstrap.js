...
document.addEventListener("keydown", handleWorkModeShortcut, true);
document.addEventListener("visibilitychange", () => void markWorkModeMessageRead());
...
chatAttachmentGuide.addEventListener("click", (event) => {
  ...
  if (!kind) return;
  if (kind === "photo" || kind === "pdf" || kind === "file") {
    event.preventDefault();
    resetAttachmentSwipe();
