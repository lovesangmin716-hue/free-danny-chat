...
function registerCoreHooks(hooks) {
  for (const [name, hook] of Object.entries(hooks)) {
    if (!(name in coreHooks) || typeof hook !== "function") {
      throw new TypeError(`Unknown Colorless core hook: ${name}`);
    }
    coreHooks[name] = hook;
  }
}
...
async function chooseStatusEmoji(emoji) {
  ...
  try {
    ...
    coreHooks.renderMessenger?.();
    coreHooks.updatePresence?.();
  } catch (error) {
...
export {
  ...
  CHAT_MESSAGE_ROW_GAP,
  CHAT_MESSAGE_VIRTUAL_OVERSCAN_PX,
  DEFAULT_PROFILE_PALETTE,
  IMAGE_DIMENSION_MAX,
  IMAGE_EDGE_PIXELS_MAX,
  IMAGE_FALLBACK_TOTAL_PIXELS_MAX,
  ...
  PROFILE_IMAGE_UPLOAD_BYTES_MAX,
  ...
  myView,
  newChatGroupName,
  newChatGroupNameField,
  newChatMemberList,
  newChatSearch,
  newChatSheet,
  normalizeProfilePixels,
  normalizeStatusEmoji,
  openCustomStatusEmojiInput,
  openDirectoryButton,
  openListSearchButton,
  openLoginButton,
  openNewChatButton,
  openProfileButton,
  openRoomSettingsButton,
  openStatusEmojiButton,
  ...
  profileDisplayName,
  profileFriendCode,
  profilePaletteSelect,
  profilePhotoCrop,
  profilePhotoCropCanvas,
  profilePhotoInput,
  profilePhotoPreview,
  profilePhotoZoom,
  ...
  removeRoomPhotoButton,
  renderStatusEmojiControl,
  requestAction,
  roomPhotoInput,
  roomSettingsName,
  roomSettingsOwnerHelp,
  roomSettingsPhotoPreview,
  roomSettingsSheet,
  saveProfileButton,
  saveProfilePhotoButton,
  saveRoomSettingsButton,
  savedStatusEmoji,
  selectCenteredStatusEmoji,
  selectProfilePhotoButton,
  selectRoomPhotoButton,
  setAppStatus,
  setAuthRequestBusy,
  setAuthStatus,
  setProviderStatus,
