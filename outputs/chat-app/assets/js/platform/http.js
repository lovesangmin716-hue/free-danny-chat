...
(function registerHttp(global) {
  ...
  function createHttpClient({ onUnauthorized } = {}) {
    async function request(url, options = {}) {
      const { headers: optionHeaders = {}, ...requestOptions } = options;
      const method = String(requestOptions.method || "GET").toUpperCase();
      const isJsonBody = typeof requestOptions.body === "string";
      const requestUrl = new URL(String(url), global.location.href);
      const isSameOrigin = requestUrl.origin === global.location.origin;
  ...
  platform.HttpError = HttpError;
  platform.createHttpClient = createHttpClient;
})(window);
